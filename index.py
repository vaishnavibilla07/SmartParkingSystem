    ...
    if command == "P":
        ...
        while ret_val == -1:
            while True:
                ...
                row = input("Select Row to Park In,If your vehicle is a truck select first row,if it is a car select from the next 2 rows and if it is a motorcycle select the last row:\n"
                            ">")
                if row.isnumeric():
                    if int(row) < rows:
                        break
